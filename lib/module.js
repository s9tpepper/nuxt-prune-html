...
*   * Debug function
*/
const debug = Debug('nuxt-prune-html:module.js')
...
function shouldPrune(headers, options) {
    const _d = debug.extend('shouldPrune')
    /*
    *   * Types
    */
    ...
    */
    if( isTypeDefaultDetect && headers && headers[ options.headerNameForDefaultDetection ] ) {
        _d(`headers[ ${options.headerNameForDefaultDetection} ]: ${headers[ options.headerNameForDefaultDetection ]}`)
        _d(JSON.stringify(headers))
        ...
        _d(`shouldPruneFromBotAuditOrMobileDetection: ${shouldPruneFromBotAuditOrMobileDetection}`)
    }
    ...
    */
    if( isTypeQueryParameters && _parsedUrl ) {
        ...
        // Should prune?
        _d(`isTypeQueryParameters :: shouldExcludePrune: ${shouldExcludePrune}`)
        ...
        _d(`shouldPruneFromQueryParameter: ${shouldPruneFromQueryParameter}`)
    ...
    */
    if( isTypeHeaders && headers ) {
        ...
        // Should prune?
        _d(`isTypeHeaders :: shouldExcludePrune: ${shouldExcludePrune}`)
        if( shouldExcludePrune )
            return false;
        ...
        _d(`shouldPruneFromHeaderExist: ${shouldPruneFromHeaderExist}`)
    }
...
export default function nuxtPruneHtml(
    ...
    */
    options.hookRenderRoute && this.nuxt.hook(
        ...
        async (
            ...
            */
            ...
            if (!shouldPrune(headers, options))
                return;
    ...
    return { pruneHtml, shouldPrune, options }
}
